...
public class ProductService {
    ...
    public void updateProduct(Long id, Products editedSalonService) {
        Products salonService = productRepository.findById(id)
                .orElseThrow(() -> new NoSuchElementException("Автомобиль не найдена"));
        if (editedSalonService.getPrice() <= 0) {
            throw new IllegalArgumentException("Цена должна быть больше 0");
        }
        salonService.setTitle(editedSalonService.getTitle());
        salonService.setDescription(editedSalonService.getDescription());
        salonService.setPrice(editedSalonService.getPrice());
        salonService.setMaster(editedSalonService.getMaster());
        productRepository.save(salonService);
    }
}
